
/**
 * Implementación de la clase AplicacionFacultad para pruebas.
 * 
 * @author Bengochea Marcelo José De Jesús
 * @author Aguilar Jonathan Alberto
 * @version 1.00
 */
import java.util.*;

public class AplicacionFacultad
{
    public static void main(String[] args){
        try{
            Cargo cargo1 = new Cargo("JTP-Programacion OO", 800, 2015);
            Cargo cargo2 = new SemiExclusivo("Titular-TFA", 1400, 2013, 8);
            Cargo cargo3 = new Cargo("JTP-Analisis de Sistemas", 800, 2005);
            Cargo cargo4 = new Cargo("JTP-II", 1200, 2014);
            Cargo cargo5 = new Exclusivo("JTP-Ing.Soft", 1800, 2015, 10, 20, 10);
            //Lanza excepcion por hs de investig fuera de rango (0~10)
            //Cargo cargo6 = new SemiExclusivo("Titular-TFA", 1400, 2013, 20);
            
            ArrayList<Cargo> listaCargos = new ArrayList<Cargo>();
            listaCargos.add(cargo1);
            listaCargos.add(cargo2);
            listaCargos.add(cargo3);
        
            ArrayList<Cargo> listaCargos2 = new ArrayList<Cargo>();
            listaCargos2.add(cargo4);
            listaCargos2.add(cargo5);
            
            Calendar fechaNac = Calendar.getInstance();
            fechaNac.set(1992, 8, 16);
            Profesor prof1 = new Profesor(21859361, "Juan", "Perez", fechaNac, "Lic. en Sistemas de Informacion", listaCargos);
            fechaNac.set(1978, 0, 12);
            Profesor prof2 = new Profesor(34519921, "Mirta", "Lopez", fechaNac, "Lic. en Sistemas de Informacion", listaCargos2);
        
            Facultad facu = new Facultad("FaCENA", prof1);
            facu.agregarProfesor(prof2);
            //facu.nominaProfesores();
            //prof1.mostrar();
            facu.listarProfesorCargos();
        } catch (IllegalArgumentException e) {
            System.out.println("\t---Error: " + e);
        } catch (Exception e){
            System.out.println("Error: " + e);
        } finally {
            System.out.println("\n\t------Programa finalizado--------");
        }
    }
}